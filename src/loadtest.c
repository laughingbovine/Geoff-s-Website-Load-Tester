...
void get_options (int argc, char **argv)
{
    ...
    cli_input_files = calloc(cli_num_input_files, sizeof(char*));
...
int main (int argc, char **argv)
{
    ...
    if (cli_num_input_files > 0) // input coming from files designated using command line args
    {
        ...
        for (i = 0; i < cli_num_input_files; i++)
            the_inputs[i].input = read_file(cli_input_files[i], buffer, BUF_SIZE);
    }
    else
    {
        printf("Please enter the message: \n");
        the_inputs[0].input = read_lines(buffer, BUF_SIZE);
        printf("Thank you.\n");
